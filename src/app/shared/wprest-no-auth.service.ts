...
export class WprestNoAuthService {
  ...
  getPosts(page){
    return this.httpClient.get(ConstantGlobals.WP_API_BASE + '/wp/v2/persons?per_page=10&page='+page, {observe: 'response'});
  }

  getPostsInCategory(categoryId,page){
    return this.httpClient.get(ConstantGlobals.WP_API_BASE + '/wp/v2/persons?categories='+categoryId+'&per_page=10&page='+page, {observe: 'response'});
  }

  getPostsInTag(tagId,page){
    return this.httpClient.get(ConstantGlobals.WP_API_BASE + '/wp/v2/persons?tags='+tagId+'&per_page=10&page='+page, {observe: 'response'});
  }

  getPostById(id){
    return this.httpClient.get(ConstantGlobals.WP_API_BASE + '/wp/v2/persons/'+id, {observe: 'response'});
  }

  getPostBySlug(slug){
    return this.httpClient.get(ConstantGlobals.WP_API_BASE + '/wp/v2/persons?slug='+slug, {observe: 'response'});
  }
